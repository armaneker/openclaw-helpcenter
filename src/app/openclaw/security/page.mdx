...
- **Coordinator** — Allow: all core tools + `sessions_spawn`. Deny: none.
- **Engineering** — Allow: `read`, `write`, `exec`, `process`, `git`. Deny: `sessions_spawn`, `gateway`, `cron`.
- **Product** — Allow: `read`, `write`, `web_search`. Deny: `exec`, `process`, `git`, `gateway`.
- **Risk / Analytics** — Allow: `read`, `write`, `web_search`. Deny: `exec`, `gateway`, `sessions_spawn`.
